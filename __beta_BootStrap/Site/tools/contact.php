<?php
	...
	if(strlen($name) < 2) {
		if(!$name) {
			...
			$errors[] = "Nome requer pelo menos dois caracteres!";
		}
	}
	if(!$email) {
		$errors[] = "Digite seu e-mail!";
	} else if(!validEmail($email)) {
		$errors[] = "Por favor insira um email válido!";
	}
	if(strlen($message) < 10) {
		if(!$message) {
			$errors[] = "Digite uma mensagem!";
		} else {
			$errors[] = "Mensagem requer pelo menos 10 caracteres!";
		}
	}
	...
	die("<p class='success'>Obrigado! - A sua mensagem foi enviada com sucesso!</p>");
